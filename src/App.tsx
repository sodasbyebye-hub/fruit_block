...
function App() {
  ...
  const activeState = mode === 'online' && online.visibleState ? online.visibleState : localGame.state;
  const activeControls =
    mode === 'online'
      ? { ...hostGame.controls, ...online.controls, dispatch: online.dispatch }
      : localGame.controls;
  const canControl = mode === 'local' || online.role === 'host';
  const isFocusLayout = mode === 'online' && Boolean(online.role && online.visibleState);
  const appClassName = `app-shell mode-${mode} ${isFocusLayout ? 'is-focus-layout' : ''}`;
  ...
  }, [localGame.controls, mode, online]);
    ...
    <main className={appClassName} style={motionStyle}>
      ...
      <section className="mode-panel" aria-label={labels.modeOnline}>
        <div className="mode-tabs">
          <button className={`mode-tab ${mode === 'local' ? 'is-selected' : ''}`} type="button" onClick={() => setMode('local')}>
            {labels.modeLocal}
          </button>
          <button className={`mode-tab ${mode === 'online' ? 'is-selected' : ''}`} type="button" onClick={() => setMode('online')}>
            {labels.modeOnline}
          </button>
        </div>
      ...
      {mode === 'online' && online.role && online.visibleState ? (
        <OnlineBattleStage state={activeState} localPlayer={online.localPlayer} labels={labels} onAction={online.dispatch} />
      ) : (
